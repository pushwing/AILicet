...
<div class="page-head" style="display:flex;justify-content:space-between;align-items:flex-end;">
    ...
    <a href="/admin/licenses/new" class="btn btn--primary">+ 라이센스 발급</a>
</div>
...
<div class="card">
    <div class="card__body">
        <div style="display:flex;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
            <input class="input" id="search" placeholder="호스트ID·상품명·코드 검색" style="max-width:280px;">
            <select class="input" id="typeFilter" style="max-width:150px;">
        ...
        <div id="licenseGrid" class="ag-theme-alpine grid-wrap" style="height:460px;"></div>
...
<script>
    ...
    const gridOptions = {
        columnDefs: [
            ...
            { field: 'period_code', headerName: '기간정책', flex: 0.8 },
            { field: 'expire_date', headerName: '만료일', flex: 0.8, valueFormatter: p => p.value || '무기한' },
            { field: 'status', headerName: '상태', flex: 0.7, cellRenderer: p => {
                const [cls, label] = STATUS[p.value] || ['muted', p.value];
                return `<span class="badge badge--${cls}">${label}</span>`;
            } },
            { headerName: '', flex: 0.6, sortable: false,
              cellRenderer: p => `<a class="btn btn--ghost" style="padding:4px 10px" href="/admin/licenses/${p.data.id}">상세</a>` },
        ],
    ...
    async function reload(page) {
        ...
        const json = await (await fetch(`/admin/licenses/data?${params}`)).json();
    ...
    api = agGrid.createGrid(document.getElementById('licenseGrid'), gridOptions);
