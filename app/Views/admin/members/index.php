...
<div class="page-head page-head--actions">
    ...
    <a href="/admin/members/new" class="btn btn--primary">+ 회원 등록</a>
</div>
...
<div class="card">
    <div class="card__body">
        ...
        <div id="memberGrid" class="ag-theme-alpine grid-wrap" style="height:460px;"></div>
...
<script>
    ...
    const TYPE_LABEL = { agency: '대행사', client: '고객' };
    ...
    function deleteMember(id, name) {
        ...
        f.method = 'post'; f.action = `/admin/members/${id}/delete`;
    ...
    const gridOptions = {
        columnDefs: [
            { field: 'customer_type', headerName: '유형', flex: 0.6,
              cellRenderer: p => `<span class="badge badge--muted">${TYPE_LABEL[p.value] || p.value}</span>` },
            ...
            { headerName: '관리', flex: 1, sortable: false, cellRenderer: p => {
                const id = p.data.id, name = (p.data.company_name || '').replace(/'/g, '\\\'');
                return `<a class="btn btn--ghost" style="padding:4px 10px" href="/admin/members/${id}/edit">수정</a>
                        <button class="btn btn--ghost" style="padding:4px 10px" onclick="deleteMember(${id}, '${name}')">삭제</button>`;
            } },
        ],
    ...
    async function reload(page) {
        ...
        const res = await fetch(`/admin/members/data?${params}`);
        const json = await res.json();
        api.setGridOption('rowData', json.data);
        state.lastPage = json.meta.last_page;
        document.getElementById('pageInfo').textContent =
            `총 ${json.meta.total}건 · ${json.meta.page}/${json.meta.last_page} 페이지`;
    ...
    api = agGrid.createGrid(document.getElementById('memberGrid'), gridOptions);
